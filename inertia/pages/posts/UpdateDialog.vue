...
import { UpdatePostData } from '#validators/post';
import { PostReadModel } from './types';

defineExpose({ open })

const dialog = ref();
const form = ref<UpdatePostData>({
  title: '',
  description: '',
})
let errors = ref<Partial<Record<keyof UpdatePostData, string[]>>>({});
let postId: number;

function open(post: PostReadModel) {
  postId = post.id;
  errors.value = {};
  form.value = {
    title: post.title,
...
function updatePost() {
  router.patch(`/posts/${postId}`, form.value, {
    preserveScroll: true,
    onSuccess: close,
